...
namespace ProjectPRNLamnthe180410.Services
{
    public class VnPayLibrary
    {
        ...
        public PaymentResponseModel GetFullResponseData(IQueryCollection collection, string hashSecret)
        {
            ...
            var vnpAmount = GetResponseData("vnp_Amount");
            ...
            // Extract username from orderInfo
            string username = orderInfo?.Split(" purchase")[0];

            return new PaymentResponseModel
            {
                Amount = string.IsNullOrEmpty(vnpAmount) ? 0 : int.Parse(vnpAmount) / 1000,
                ContentTitle = orderInfo, 
                Username = username,
                Email = GetResponseData("vnp_Bill_Email"),
                Locale = GetResponseData("vnp_Locale"),
